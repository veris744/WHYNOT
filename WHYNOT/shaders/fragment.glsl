...
void main()
{
    ...
    for (int i = 0; i < uNumLights; i++)
    {
        ...
        {
            // Ambient
            ambient = uLights[0].color * uLights[0].ambient * vec3(texture(uAmbient, aUV));
            continue;
        }
        ...
        if (uLights[i].type == 0)
        {
            v_LightDir = normalize(-uLights[i].direction);
        }
        else
        {
            v_LightDir      = normalize(uLights[i].position - aFragPosition);
        ...
        if (uLights[i].type == 2)
        {
            f_DotProduct = dot(v_LightDir, normalize(-uLights[i].direction));
        ...
        if (uLights[i].type != 2 || (uLights[i].type == 2 && f_DotProduct > f_OuterCuttoff))
        {
            ...
            diffuse         = diff * uLights[i].color * uLights[i].diffuse * vec3(texture(uDiffuse, aUV)) * f_Attenuation * f_Intensity;
            
            // Specular
            vec3 viewDir    = normalize(uViewPos - aFragPosition);
            vec3 reflectDir = reflect(-v_LightDir, aNormal);
            float spec      = pow(max(dot(viewDir, reflectDir), 0.0), uShininess);
            specular        = uLights[i].color * uLights[i].specular * spec * vec3(texture(uSpecular, aUV)) * f_Attenuation * f_Intensity;
