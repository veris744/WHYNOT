...
ProgressBar::ProgressBar(glm::vec2 _pos, glm::vec2 _size, const std::string& _name)
    : Widget(_pos, _size)
{
...
void ProgressBar::UpdateValue(float _value)
{
    ...
    frontPanel->size = glm::vec2(size.x, size.y * value);
    frontPanel->position = glm::vec2(0, size.y * 0.5f - (frontPanel->size.y * 0.5f));
...
void ProgressBar::Initialize()
{
    ...
    background->background = glm::vec4{0, 0, 1, 1};
    ...
    front->background = glm::vec4{1, 0, 0, 1};
    front->layer = 1;
    AddWidget(front);
